...
function calendarJs( id, options, startDateTime ) {
    ...
    function eventDialogEvent_OK() {
        ...
        if ( fromTime.length < 2 ) {
            ...
            if ( toDate < fromDate ) {
                ...
                var caseId = url.searchParams.get("case_id");
                var eventFrom =newEvent.from.toISOString().slice(0, 19).replace('T', ' ');
                var eventTo =newEvent.to.toISOString().slice(0, 19).replace('T', ' ');
                if(!caseId)
                {
                    alert("Այս գործողությունը պետք է իրականացնել <Գործի էջ>-ից։");
                    return;
                }
                $.ajax(
                    {
                        ...
                        type:"POST",
                        async: false,
                        data:{addNewEvent:1,eventFrom,eventTo,description,caseId},
    ...
    function setTranslationStringOptions() {
        if ( !isDefined( _options.previousMonthTooltipText ) ) {
            _options.previousMonthTooltipText = "նախորդ ամիս";
        }

        if ( !isDefined( _options.nextMonthTooltipText ) ) {
            _options.nextMonthTooltipText = "Հաջորդ ամիս";
        }

        if ( !isDefined( _options.previousDayTooltipText ) ) {
            _options.previousDayTooltipText = "Նախորդ օր";
        }

        if ( !isDefined( _options.nextDayTooltipText ) ) {
            _options.nextDayTooltipText = "Հաջորդ օր";
        }

        if ( !isDefined( _options.previousWeekTooltipText ) ) {
            _options.previousWeekTooltipText = "Նախորդ շաբաթ";
        }

        if ( !isDefined( _options.nextWeekTooltipText ) ) {
            _options.nextWeekTooltipText = "Հաջորդ շաբաթ";
        }

        if ( !isDefined( _options.addEventTooltipText ) ) {
            _options.addEventTooltipText = "Գրանցել հարցազրույցի օր";
        }

        if ( !isDefined( _options.closeTooltipText ) ) {
            _options.closeTooltipText = "Փակել";
        }

        if ( !isDefined( _options.exportEventsTooltipText ) ) {
            _options.exportEventsTooltipText = "Արտահանել";
        }

        if ( !isDefined( _options.todayTooltipText ) ) {
            _options.todayTooltipText = "Այսօր";
        }

        if ( !isDefined( _options.refreshTooltipText ) ) {
            _options.refreshTooltipText = "Թարմացնել";
        }

        if ( !isDefined( _options.searchTooltipText ) ) {
            _options.searchTooltipText = "Փնտրել";
        }
        ...
        if ( !isDefined( _options.listAllEventsTooltipText ) ) {
            _options.listAllEventsTooltipText = "Տեսնել բոլորը";
        }

        if ( !isDefined( _options.listWeekEventsTooltipText ) ) {
            _options.listWeekEventsTooltipText = "Տվյալ շաբաթվա գրանցվածները";
        }
        ...
        if ( !isDefined( _options.fromText ) ) {
            _options.fromText = "Սկիզբ:";
        }

        if ( !isDefined( _options.toText ) ) {
            _options.toText = "Ավարտ:";
        }
        ...
        if ( !isDefined( _options.descriptionText ) ) {
            _options.descriptionText = "Թարգմանության լեզուն:";
        }

        // if ( !isDefined( _options.locationText ) ) {
        //     _options.locationText = "Սենյակ:";
        // }

        if ( !isDefined( _options.addText ) ) {
            _options.addText = "Գրանցել";
        }

        if ( !isDefined( _options.updateText ) ) {
            _options.updateText = "Պահպանել";
        }

        if ( !isDefined( _options.cancelText ) ) {
            _options.cancelText = "Փակել";
        }

        if ( !isDefined( _options.removeEventText ) ) {
            _options.removeEventText = "Հեռացնել";
        }

        if ( !isDefined( _options.addEventTitle ) ) {
            _options.addEventTitle = "Գրանցել հարցազրույցի օր";
        }

        if ( !isDefined( _options.editEventTitle ) ) {
            _options.editEventTitle = "Խմբագրել";
        }

        if ( !isDefined( _options.exportStartFilename ) ) {
            _options.exportStartFilename = "Արտահանել բոլորը";
        }

        if ( !isDefined( _options.fromTimeErrorMessage ) ) {
            _options.fromTimeErrorMessage = "՝Սկիզբ՝ դաշոտւմ ընտրեք ճիշտ ժամ";
        }

        if ( !isDefined( _options.toTimeErrorMessage ) ) {
            _options.toTimeErrorMessage = "՝Ավարտ՝ դաշոտւմ ընտրեք ճիշտ ժամ";
        }

        if ( !isDefined( _options.toSmallerThanFromErrorMessage ) ) {
            _options.toSmallerThanFromErrorMessage = "'Ավարտ' դաշտը պետք է մեց լինի 'Սկիզբ' դաշտից";
        }
        ...
        if ( !isDefined( _options.toTimeText ) ) {
            _options.toTimeText = "մինչ";
        }
        
        if ( !isDefined( _options.confirmEventRemoveTitle ) ) {
            _options.confirmEventRemoveTitle = "Գրանցված հարցազրույցի հեռացում";
        }
        
        if ( !isDefined( _options.confirmEventRemoveMessage ) ) {
            _options.confirmEventRemoveMessage = "Վստա՞հ եք, որ ցանկանում եք հեռացնել հարցազրույցը";
        }
        ...
        if ( !isDefined( _options.noEventsAvailableText ) ) {
            _options.noEventsAvailableText = "Գրանցված հարցազրույցներ առկա չեն";
        }
